...
EXCEL_FILE = r"C:\Users\TDutta\derivasense\public\python_optionchain_website_mod1.xlsm"
...
def process_symbol_bse(cfg):
    ...
    print(f"[{symbol}] Option Expiry:", option_expiry, "| Future Expiry:", future_expiry)

    session = bse_session()

    option_json = fetch_bse_option_chain(session, cfg["scrip_cd"], option_expiry)

    table = option_json.get("Table") or []

    if not table:
        print(f"[{symbol}] No data returned from BSE for expiry {option_expiry}.")
        return

    spot = to_number_bse(table[0].get("UlaValue", ""))

    initialize_atm_strikes(symbol, spot, cfg["strike_step"])

    future, vwap = fetch_bse_future(session, future_expiry)

    rows = build_option_rows_bse(option_json)

    # Same reasoning as process_symbol(): export to the dashboard before any
    # Excel writes, so a busy/blocked Excel can't block SENSEX's dashboard
    ...
    print(f"[{symbol}] Spot={spot} Future={future} Rows={len(rows)}")
